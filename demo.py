    ...
    with torch.no_grad():
        for i, result in enumerate(val_dataloader):
            ...
            if opt.warp_cloth:
                ...
                warped_cloth_path = os.path.join(
                    'dataset', 'warped_cloth', warped_cloth_name[0])
            ...
            generate_parse_argmax = torch.argmax(
                generate_parse, dim=1, keepdim=True).float()
            res = []
            ...
            "A"
            image_without_cloth = create_part(
                source_image, source_parse, 'image_without_cloth', False)
            input_app = torch.cat(
            ...
            generate_face = create_part(
                refine_img, generate_parse_argmax, 'face', False)
            generate_img_without_face = refine_img - generate_face
            source_face = create_part(
                source_image, source_parse, 'face', False)
            input_face = torch.cat((source_face, generate_face), 1)
            fake_face = generator_face(input_face)
            fake_face = create_part(
                fake_face, generate_parse_argmax, 'face', False)
            ...
            if opt.save_time:
                ...
                generate_parse_vis = torch.argmax(
                    generate_parse, dim=1, keepdim=True).permute(0, 2, 3, 1).contiguous()
                generate_parse_vis = pose_utils.decode_labels(
                    generate_parse_vis)
            "save results"
            images = [source_image, cloth_image, target_pose_img, warped_cloth,
