...
def array2img(img_array):
    imgs = (img_array.permute(
        0, 2, 3, 1).contiguous().cpu().numpy() * 0.5 + 0.5) * 255
    ...
    for i in range(len(images)):
        if isinstance(images[i], np.ndarray):
            ...
            if images[i].shape[1] == 3:
                images[i] = ((images[i].permute(0, 2, 3, 1).contiguous(
                ).cpu().numpy() * 0.5 + 0.5) * 255).astype(np.uint8)
...
def save_img_grid(images, path):
    ...
    for i in range(len(images)):
        if isinstance(images[i], np.ndarray):
            images[i] = torch.from_numpy(images[i].astype(
                np.float32)).permute(0, 3, 1, 2).contiguous()
...
def cords_to_map(cords, img_size, sigma=100):
    ...
    for i, point in enumerate(cords):
        if point[0] == MISSING_VALUE or point[1] == MISSING_VALUE:
            continue
        xx, yy = np.meshgrid(
            np.arange(img_size[1]), np.arange(img_size[0]))   # 192, 256
...
def compute_interpol_map(inp_map, tg_map):
    ...
    for i in range(1, num_stacks+1):
        interpol_pose = compute_interpol_pose(
            inp_pos, tg_pos, i, num_stacks, pose_dim)
        # print(interpol_pose)
...
def compute_interpol_cord_warp(inp_map, interpol_pose):
    ...
    for pose in interpol_pose:
        ...
        if _warp_skip == 'mask':
            ...
            warp[1] = pose_transform.pose_masks(
                kp_array2, _image_size, pose_dim)
        else:
    ...
    # bad logic to circumvent missing annot . . synthesize and vanish missing annot after half sequence is completed
        ...
        for i in range(pose_dim):
            ...
            # normal interpol when annot are present in both cases
                ...
                interpol_pose[i] = inp_pos[i] + \
                    (tg_pos[i]-inp_pos[i])*index/num_stacks
